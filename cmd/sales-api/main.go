...
func run() error {
	...
	go func() { //localhost:4000/debug/vars
	...
	select {
	case err := <-serverErrors: //blocker
		return errors.Wrap(err, "server error")

	case sig := <-shutdown: //blocker
		...
		err := api.Shutdown(ctx) //this is the blocking call/point of Guarantee - ListenAndServe will return and that's why we need a buffer of 1 (ln 108)
		if err != nil {
